...
@router.post(
    "/documents", response_model=OperationResponse, status_code=status.HTTP_201_CREATED
)
async def create_document(
    document: DocumentCreate, service: VectorService = Depends(get_vector_service)
) -> OperationResponse:
    """Create a new document in the vector database"""
    try:
        created_doc = await service.create_document(document)
        return OperationResponse(
            id=created_doc.id, status="success", message="Document created successfully"
        )
...
async def get_document(
    document_id: str, service: VectorService = Depends(get_vector_service)
) -> DocumentResponse:
...
async def update_document(
    ...
    try:
        updated_doc = await service.update_document(document_id, update)
        return OperationResponse(
            id=updated_doc.id, status="success", message="Document updated successfully"
        )
    except DocumentNotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except ChromaDBError as e:
...
async def delete_document(
    ...
    try:
        ...
        return OperationResponse(
            id=document_id, status="success", message="Document deleted successfully"
        )
    except DocumentNotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except ChromaDBError as e:
...
async def search_documents(
    query: QueryRequest, service: VectorService = Depends(get_vector_service)
) -> QueryResponse:
...
async def create_documents_batch(
    batch: BatchCreateRequest, service: VectorService = Depends(get_vector_service)
) -> BatchCreateResponse:
    """Create multiple documents in batch"""
    try:
        results = await service.create_documents_batch(batch.documents)
...
async def delete_all_documents(
    confirm: bool = Query(
        default=False, description="Confirm deletion of all documents"
    ),
    service: VectorService = Depends(get_vector_service),
) -> Dict[str, str]:
