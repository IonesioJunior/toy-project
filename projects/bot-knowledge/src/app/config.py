...
class Settings(BaseSettings):
    app_name: str = Field(
        default="FastAPI ChromaDB API", description="Application name"
    )
    ...
    max_page_size: int = Field(
        default=100, description="Maximum page size for pagination"
    )
    default_page_size: int = Field(
        default=10, description="Default page size for pagination"
    )
    ...
    enable_file_manager: bool = Field(
        default=True, description="Enable file manager integration for document storage"
    )
