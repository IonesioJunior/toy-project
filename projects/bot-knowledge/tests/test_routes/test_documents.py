...
class TestDocumentRoutes:
    ...
    def test_create_document_success(self, client, sample_document, mock_vector_service):
        """Test successful document creation"""
        with patch('src.app.routes.documents.VectorService', return_value=mock_vector_service):
            response = client.post(
                "/api/v1/documents",
                json=sample_document.model_dump()
            )
    ...
    def test_create_document_already_exists(self, client, sample_document, mock_vector_service):
        """Test document creation when document already exists"""
        mock_vector_service.create_document.side_effect = DocumentAlreadyExistsError(sample_document.id)
    ...
    def test_get_document_not_found(self, client, mock_vector_service):
        ...
        mock_vector_service.get_document.side_effect = DocumentNotFoundError("nonexistent")
    ...
    def test_batch_create_documents(self, client, sample_documents, mock_vector_service):
        """Test batch document creation"""
        batch_data = {
            "documents": [doc.model_dump() for doc in sample_documents[:3]]
        }
    ...
    def test_delete_all_documents_without_confirmation(self, client, mock_vector_service):
